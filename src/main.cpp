...
$execute
{
	...
	new EventListener<NewRPCFilter>([](std::string const &newRPCStr)
		...
		return ListenerResult::Propagate; }, NewRPCFilter("update_rpc"_spr));
...
class $modify(MyPlayLayer, PlayLayer)
{
	...
	void updateRP(bool resetTime = false)
	{
		...
		bool isRobTopLevel = ((m_level->m_levelID.value() < 128 && m_level->m_levelID.value() != 0) ||
		...
		{
			state = std::string(m_level->m_levelName) + " by " +
